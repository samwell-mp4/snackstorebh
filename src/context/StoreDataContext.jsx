...
export function StoreDataProvider({ children }) {
  ...
  const updateOrderStatus = async (id, status) => {
    ...
    const orderNum = typeof status === 'object' ? status?.order_number : null;
    const matchOrder = o => (
      o.id === id ||
      String(o.id) === String(id) ||
      o.order_number === id ||
      (orderNum && o.order_number === orderNum)
    );

    setOrders(prev => prev.map(o => matchOrder(o) ? { ...o, status: statusVal } : o));
    const updated = await apiService.updateOrderStatus(id, status);
    if (updated) {
      setOrders(prev => prev.map(o => matchOrder(o) ? { ...o, ...updated, status: statusVal || updated.status } : o));
    }
    await loadData();
    return updated;
  };
  ...
  const updateOrder = async (id, orderData) => {
    ...
    setOrders(prev => prev.map(o => matchOrder(o) ? { ...o, ...orderData } : o));
    const updated = await apiService.updateOrder(id, orderData);
    if (updated) {
      setOrders(prev => prev.map(o => matchOrder(o) ? { ...o, ...orderData, ...updated } : o));
    }
    await loadData();
    return updated;
  };
